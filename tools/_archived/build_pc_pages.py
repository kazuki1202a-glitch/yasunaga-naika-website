...
#
# 2026-09-14、社長判断で「PC版を採用」が確定し、スマホ用(*.html)とPC用(*_pc.html)
# の2ファイル体制を1ファイルのレスポンシブに統合しました。
# 統合作業では、このスクリプトの transform() を「サイト内リンクを _pc.html に
# 差し替えない」状態で本番ページに適用することで、14ページを機械的に統合しています。
# （index.html だけは手作りだったため、index_pc.html のリンクを本番名へ戻して採用）
# 統合後は *_pc.html が存在しないため、このスクリプトは動きません。
# 当時の変換ルール（PCナビ・pc-main・pc-wide 等の内訳）の記録として残しています。
# ============================================================================
